...
// Mapa de respostas pendentes da IA: phone → resolve
...
process.on('message', async (msg) => {
  ...
  if (msg.action === 'ai-reply' && msg.phone && msg.reply) {
    const resolve = aiPending.get(msg.phone);
    if (resolve) {
      aiPending.delete(msg.phone);
      resolve(msg.reply);
    }
  }
});
...
async function handleAiResponse(fromPhone, text, sock) {
  try {
    if (!process.send) return; // sem canal IPC, ignora

    const reply = await new Promise((resolve) => {
      aiPending.set(fromPhone, resolve);
      process.send({ action: 'ai-process', unitId, phone: fromPhone, message: text });
      // timeout de 30s — evita que a Promise fique presa
      setTimeout(() => {
        if (aiPending.has(fromPhone)) {
          aiPending.delete(fromPhone);
          resolve(null);
        }
      }, 30000);
    });
    ...
    if (reply && sock) {
      ...
      // Só salva no inbox após confirmação de envio pelo WhatsApp
      await saveOutboundMessage(fromPhone, reply);
    }
  } catch (err) {
