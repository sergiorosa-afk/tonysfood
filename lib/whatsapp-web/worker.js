...
async function handleAiResponse(fromPhone, text, sock) {
  try {
    ...
    console.log(`[worker-ai] iniciando para ${fromPhone} → ${apiBase}`);
    ...
    if (!res.ok) {
      ...
      console.log(`[worker-ai] HTTP ${res.status} — ${errBody}`);
      return;
    }

    const data = await res.json();
    console.log(`[worker-ai] reply: ${JSON.stringify(data)}`);

    if (data.reply && sock) {
      const jid = `${fromPhone.replace(/\D/g, '')}@s.whatsapp.net`;
      await sock.sendMessage(jid, { text: data.reply });
      console.log(`[worker-ai] enviado para ${jid}`);
    }
  } catch (err) {
    console.log(`[worker-ai] erro: ${err.message}`);
  }
}
