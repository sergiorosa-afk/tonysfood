...
process.on('message', async (msg) => {
  ...
  if (msg.action === 'ai-reply' && msg.phone && msg.reply) {
    ...
    try {
      // Usa remoteJid original se disponível (cobre casos @lid)
      // Caso contrário reconstrói com @s.whatsapp.net
...
function handleAiResponse(fromPhone, remoteJid, text) {
  ...
  // remoteJid é o JID original (pode ser @s.whatsapp.net ou @lid) — usado
  // no retorno para garantir que a resposta vai para o JID correto.
  try {
...
async function start() {
  ...
  sock.ev.on('messages.upsert', async ({ messages, type }) => {
    ...
    for (const msg of messages) {
      ...
      handleAiResponse(fromPhone, remoteJid, text);
    }
  });
}
