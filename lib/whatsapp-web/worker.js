...
// Handle commands from parent via IPC
...
async function handleAiResponse(fromPhone, text, sock) {
  try {
    // Sempre usa localhost para evitar round-trip externo no Railway
    const apiBase = 'http://localhost:3000';
    process.stderr.write(`[worker] AI: chamando ${apiBase}/api/whatsapp/ai-message para ${fromPhone}\n`);

    const res = await fetch(`${apiBase}/api/whatsapp/ai-message`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        'x-worker-secret': process.env.NEXTAUTH_SECRET || '',
      },
      body: JSON.stringify({ unitId, phone: fromPhone, message: text }),
    });

    if (!res.ok) {
      const errBody = await res.text().catch(() => '');
      process.stderr.write(`[worker] AI: resposta HTTP ${res.status} — ${errBody}\n`);
      return;
    }

    const data = await res.json();
    process.stderr.write(`[worker] AI: reply recebido — ${JSON.stringify(data)}\n`);

    if (data.reply && sock) {
      const jid = `${fromPhone.replace(/\D/g, '')}@s.whatsapp.net`;
      await sock.sendMessage(jid, { text: data.reply });
      process.stderr.write(`[worker] AI: mensagem enviada para ${jid}\n`);
    }
  } catch (err) {
    process.stderr.write(`[worker] AI handler error: ${err.message}\n`);
  }
}
...
async function start() {
  ...
  sock.ev.on('messages.upsert', async ({ messages, type }) => {
    ...
    for (const msg of messages) {
      ...
      await handleAiResponse(fromPhone, text, currentSock);
    }
  });
}
