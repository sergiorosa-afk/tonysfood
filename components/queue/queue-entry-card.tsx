...
import { AlocarMesaModal, type AlocacaoConfirmada } from '@/components/mesas/alocar-mesa-modal'
...
export function QueueEntryCard({ entry }: { entry: QueueEntry }) {
  ...
  function handleConfirmAlocacao(alocacao: AlocacaoConfirmada) {
    setShowModal(false)
    run('call', () =>
      callGuest(
        entry.id,
        alocacao.mesaNumero,
        alocacao.mesaId,
        alocacao.mesaGrupoId,
        alocacao.numerosGrupo,
      )
    )
  }
  ...
  return (
    <>
          ...
          <div className="flex items-start gap-4">
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="flex items-center gap-3 mt-1 flex-wrap">
                ...
                {ChanIcon && <span className="text-slate-400"><ChanIcon className="w-3.5 h-3.5" /></span>}
              </div>

              {entry.guestPhone && <p className="text-xs text-slate-400 mt-1">{entry.guestPhone}</p>}
      ...
      <AlocarMesaModal
        ...
        onConfirm={handleConfirmAlocacao}
        unitId={entry.unitId}
        guestName={entry.guestName}
        partySize={entry.partySize}
        isConfirming={loading('call')}
      />
    </>
  )
}
