...
import { Users, Clock, Phone, MessageCircle, UserCheck, Loader2, ArrowRight, XCircle, UtensilsCrossed } from 'lucide-react'
...
export function QueueEntryCard({ entry }: { entry: QueueEntry }) {
  ...
  function handleCallConfirm() {
    const num = parseInt(mesaInput, 10)
    if (!mesaInput || isNaN(num) || num < 1) {
      setMesaError('Informe um número de mesa válido')
      return
    }
    setMesaError('')
    setShowCallForm(false)
    run('call', () => callGuest(entry.id, num))
  }
  ...
  return (
    <div className={`relative rounded-xl border ${cardBorder} shadow-sm overflow-hidden transition-all`}>
      ...
      <div className="p-4">
        ...
        {/* Call form — informar número da mesa antes de chamar */}
        {showCallForm && (
          <div className="mt-4 p-3 bg-blue-50 border border-blue-200 rounded-lg space-y-2">
            ...
            <input
              ...
              min={1}
              value={mesaInput}
              onChange={(e) => { setMesaInput(e.target.value); setMesaError('') }}
              onKeyDown={(e) => { if (e.key === 'Enter') handleCallConfirm() }}
              placeholder="Ex: 5"
              autoFocus
              className="w-full px-2.5 py-2 text-sm bg-white border border-blue-200 rounded-lg text-slate-800 focus:outline-none focus:ring-2 focus:ring-blue-400"
            />
            {mesaError && <p className="text-xs text-red-600">{mesaError}</p>}
            <div className="flex gap-2">
              <button
                onClick={handleCallConfirm}
                disabled={isPending}
                className="flex-1 inline-flex items-center justify-center gap-1.5 px-3 py-2 rounded-lg bg-blue-600 hover:bg-blue-700 text-white text-xs font-semibold transition-colors disabled:opacity-50"
              >
                {loading('call') ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <ArrowRight className="w-3.5 h-3.5" />}
                Chamar
              </button>
              <button
                onClick={() => { setShowCallForm(false); setMesaInput(''); setMesaError('') }}
        ...
        {/* Actions */}
        {(isWaiting || isCalled) && !showAbandon && !showCallForm && (
          <div className="flex items-center gap-2 mt-4 flex-wrap">
            {isWaiting && (
              <button
                onClick={() => { setMesaInput(''); setMesaError(''); setShowCallForm(true) }}
                disabled={isPending}
