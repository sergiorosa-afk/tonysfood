...
export function ComposerWithAI({ conversationId }: { conversationId: string }) {
  ...
  function applyText(reply: string) {
    setText(reply)
    setShowQuick(false)
  ...
  return (
    <div className="border-t border-slate-200 bg-white shrink-0">
      {/* AI suggestions */}
      <AIReplySuggestions conversationId={conversationId} onSelect={applyText} />

      {/* Quick replies */}
      {showQuick && (
        <div className="px-4 pt-2 pb-1 flex flex-wrap gap-1.5 border-t border-slate-100">
          {QUICK_REPLIES.map((reply) => (
            <button
              key={reply}
              onClick={() => applyText(reply)}
